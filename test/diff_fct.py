...
learning_rate = ti.field(float, shape=())
learning_rate[None] = 0.1

F = ti.field(float, shape=(2 * tmax + 1, imax + 2, jmax + 2), needs_grad=True)
Ftd_x = ti.field(float, shape=(tmax, imax + 2, jmax + 2), needs_grad=True)
Ftd_y = ti.field(float, shape=(tmax, imax + 2, jmax + 2), needs_grad=True)
Ftarget = ti.field(float, shape=(imax + 2, jmax + 2))
Fd = ti.field(float, shape=(3*(imax + 2), jmax + 2))

ax = ti.field(float, shape=(tmax, imax + 2, jmax + 2),needs_grad=True)
ay = ti.field(float, shape=(tmax, imax + 2, jmax + 2), needs_grad=True)
cx = ti.field(float, shape=(tmax, imax + 2, jmax + 2), needs_grad=True)
cy = ti.field(float, shape=(tmax, imax + 2, jmax + 2), needs_grad=True)
rp_x = ti.field(float, shape=(tmax, imax + 2, jmax + 2), needs_grad=True)
rm_x = ti.field(float, shape=(tmax, imax + 2, jmax + 2), needs_grad=True)
rp_y = ti.field(float, shape=(tmax, imax + 2, jmax + 2), needs_grad=True)
rm_y = ti.field(float, shape=(tmax, imax + 2, jmax + 2), needs_grad=True)
...
def init_uv():
    ...
    for i, j in ti.ndrange((imin, imax + 1), (jmin, jmax + 1)):
        ...
        u[i, j] = - ti.sin(ux) ** 2 * ti.sin(2 * uy) * (Lx*1.0/dt/tmax/3) #(Lx/nx/dt)
        v[i, j] = ti.sin(vy) ** 2 * ti.sin(2 * vx) * (Lx*1.0/dt/tmax/3) # (Lx/nx/dt)
...
def get_vof(t:ti.i32):
    ...
    for i, j in ti.ndrange(imax + 2, jmax + 2):
        Fd[i, j] = F[t, i, j]
    # Copy the target field to display        
    for i, j in ti.ndrange(imax + 2, jmax + 2):
        Fd[i + imax + 2, j] = Ftarget[i, j]
...
def get_vof_grad():
    # Copy the target field to display        
    for i, j in ti.ndrange(imax + 2, jmax + 2):
        Fd[i + 2 * imax + 4, j] = F.grad[0, i, j] 


@ti.kernel
def compute_loss():
    for i, j in ti.ndrange((imin, imax + 2), (jmin, jmax + 2)):
        loss[None] += ti.abs(Ftarget[i, j] - F[2 * tmax, i, j])  # maybe ti.abs is required


@ti.kernel
def apply_grad():
    for i, j in ti.ndrange(imax + 2, jmax + 2):
        # if ti.abs(F.grad[0, i, j]) < 2.:
        F[0, i, j] -= learning_rate[None] * F.grad[0, i, j]
        F[0, i, j] = var(0, 1, F[0, i, j])
...
def forward(eps_value):
    ...
    for istep in range(tmax):
        ...
        if (istep % nstep) == 0:  # Output data every <nstep> steps
            ...
            plot_contour(Fd)
            gui.show(f'./output/opt-{opt:03d}-step-{istep:05d}.png')
    compute_loss()
    print(f'>>> >>> Current loss: {loss[None]}')
